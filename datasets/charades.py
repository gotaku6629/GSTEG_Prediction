...
class Charades(data.Dataset):
    def __init__(self, rgb_root, split, labelpath, cachedir, future, temporal, rgb_transform=None, target_transform=None):
    ...
    # temporal       = '3'
        ...
        self.data = cache(cachename)(self.prepare)(rgb_root, self.labels, split, future, temporal)


    def prepare(self, rgb_path, labels, split, future, temporal):
        print('Charades prepare')
        FPS, GAP, testGAP = 24, 4, self.testGAP
        rgb_datadir = rgb_path
        STACK= 10 + 4 * (temporal -1)
        ...
        for i, (vid, label) in enumerate(labels.items()): # num_(kind of video)
            ...
            if split == 'val_video':
                ...
                spacing = np.linspace(0, n-1-fidx, testGAP) # (1つのビデオ-44)を25分割

                for loc in spacing:
                    # get image x 3
                    rgb_temporal = []
                    for t in range(temporal):
                        rgb_impath = '{}/{}-{:06d}.jpg'.format(rgb_iddir, vid, int(np.floor(loc))+4*t+1) # int(np.floor(loc))番目
                        rgb_temporal.append(rgb_impath)    
                    rgb_image_paths.append(rgb_temporal) # 1つのVideoを25分割したそれぞれの画像
                    
                    # get labels
                    # その画像に対して先頭画像から44Flame先(予測としては27フレーム先)のラベルをappendする
                    for x in label: # label of 1 video
                    # [{'scene': 9, 'class': 'c092', 'start': 11.9, 'end': 21.2},
                    #  {'scene': 9, 'class': 'c147', 'start':  0.0, 'end': 12.6}]
                        if x['start'] < int(np.floor(loc))+fidx/float(FPS) < x['end']: # FPS=24
                    ...
                    ids.append(vid)
                    times.append(int(np.floor(loc))+fidx)

            else: # train, test                
                # get image x 3        
                for ii in range(0, n-1, GAP):
                    rgb_temporal = []
                    for t in range(temporal): # get 3 image 
                        rgb_impath = '{}/{}-{:06d}.jpg'.format(rgb_iddir, vid, ii+4*t+1)
                        rgb_temporal.append(rgb_impath)
                    if ii > n - STACK - fidx: # fidx=future+STACK-1=44
                        continue                    
                    rgb_image_paths.append(rgb_temporal)

                # get labels {s,v,o,ids,time}
                for ii in range(0, n-1, GAP): # GAP=4, num_(1 video length)
                    if ii < fidx: # pass the labels
                        continue
                    ...
                    # [{'scene': 9, 'class': 'c092', 'start': 11.9, 'end': 21.2},
                    #  {'scene': 9, 'class': 'c147', 'start':  0.0, 'end': 12.6}]
                        if x['start'] < ii/float(FPS) < x['end']: # FPS=24
                            o, v = cls2int(x['class'], self.c2ov)
                            o_target[o] = 1
                            v_target[v] = 1                       
                        ## s is single label
    ...
    def __getitem__(self, index):
        """
        Args:
            index (int): Index # Indexから10枚の画像を連続してとっていく
        Returns:
        ...
        for t in range(self.temporal):
            ...
            for i in range(rgb_STACK):
                _img = '{}{:06d}.jpg'.format(rgb_base, rgb_framer+i)
        ...
        rgb_3image = torch.cat([rgb_temporal[i].unsqueeze(0) for i in range(len(rgb_temporal))], dim=0)

        # 3.3 select anotation
        ...
        return rgb_3image, s_target, o_target, v_target, meta
...
def get(args):
    ...
    train_dataset = Charades(
        args.rgb_data, 'train', train_file, args.cache_dir, future, temporal,
    ...
    val_dataset = Charades(
        args.rgb_data, 'val', val_file, args.cache_dir, future, temporal,
    ...
    valvideo_dataset = Charades(
        args.rgb_data, 'val_video', val_file, args.cache_dir, future, temporal,
        rgb_transform=transforms.Compose([
            transforms.Resize(int(256./224*args.inputsize)),
            transforms.CenterCrop(args.inputsize),
            transforms.ToTensor(),
