...
class Trainer():
    def train(self, loader, base_model, logits_model, criterion, base_optimizer, logits_optimizer, epoch, args, writer):
        ...
        logits_model.train() # Linear Networks
        criterion.train()    # Inference
        ...
        for i, (input, s_target, o_target, v_target, meta) in enumerate(part(loader)):
            # input = [10, 3, 3, 10, 224, 224] ここだけ3時刻分
            # s_target = [10] 目標は1つ
            # o_target = [10, 38]
            # v_target = [10, 33]
            # meta = {id:[10], time:[10]}
            ...
            s_target = s_target.long().cuda()
            o_target = o_target.long().cuda()
            v_target = v_target.long().cuda()
            ...
            #print('input_var.shape={}'.format(input_var.shape)) # [10, 3, 3, 10, 224, 224]

            # 1.I3D Networks
            #print('3.1.1 base_model')
            feat = base_model(input_var)
            #print('feat.shape={}'.format(feat.shape)) # [10, 3, 1024]
            feat = feat.transpose(0,1)
            #print('feat.shape={}'.format(feat.shape)) # [3, 10, 1024]
            # 2.Linear Networks
            #print('3.1.2 logit_model')
            s, o, v, so, ov, vs, ss, oo, vv, so_t, ov_t, vs_t, os_t, vo_t, sv_t = logits_model(feat)
            #print('s') # [3, 10, 16]
            #print('so') # [3, 10, 16, 38]
            
            # 3.inference
            #print('3.1.3 inference')
            ...
            #print('s_output') #[0.0023, 001872, 009831, 01023, ...]
            #print('s_target') #[0, 0, 0, 0, 1, 0, 0,...]
            #print('v_output')
            #print('v_target')
            #print('o_output')
            #print('o_target')
            ...
            if i % args.print_freq == 0:
                ...
                writer.writerow([i, losses.val.to('cpu').detach().clone().numpy(), s_top1.val.to('cpu').detach().clone().numpy(), o_top1.val.to('cpu').detach().clone().numpy(), v_top1.val.to('cpu').detach().clone().numpy()])
    ...
    def validate_video(self, loader, base_model, logits_model, criterion, epoch, args):
        ...
        with torch.no_grad():
            ...
            """
            # divide 1 video into 25 flame x 10
            #   the index of the image is putting about future 44 flame labels
            # input       ... current 25 flame
            # s,v,o, meta ... future  25 flame
            """
            for i, (input, s_target, o_target, v_target, meta) in enumerate(loader):
                # input = [25, 3, 10, 224, 224]
                # s_target = [25]
                # o_target = [25, 38]
                # v_target = [25, 33]
                # meta = { id  : [25,1]
